...
runGA <- function(noRuns = 30, problem = "tsp"){
  ...
  if (problem == "TSP"){
    ...
    maxGenerations <<- 200
    popSize = 100 #10,50,100 - 500,100,100 - 100,100,100
    pcrossover = 0.4 #1,0,1  - 0.5,1,0.1 - 1,0,5,0.9
    crossover = gaperm_oxCrossover #gaperm_pbxCrossover gaperm_pmxCrossover gaperm_oxCrossover
    Elitism = 2
  
    pmutation = 0.2 #1,0,1 - 0.5,1,0.2 - 1,0.6,0.9
    
    run = 100 #500 - 500 - 100  - 100
    type = "permutation"
    data = getData()
  ...
  for (i in 1:noRuns){
    ...
    if (problem == "feature")
      ...
      GA <- ga(type = type, fitness = fitness, distMatrix = data, 
                     ...
                     run = run, pcrossover=pcrossover,crossover = crossover, pmutation = pmutation, monitor= monitor, seed = i )
